...
(function () {
  ...
  function band(g, cx, y, hw, sag) {
    g.beginPath();
    g.moveTo(cx - hw, y);
    g.quadraticCurveTo(cx, y + sag, cx + hw, y);
    g.stroke();
  }
  ...
  function dome(g, cx, baseY, rx, ry) {
    g.beginPath();
    g.ellipse(cx, baseY, rx, ry, 0, Math.PI, Math.PI * 2, false);
    g.stroke();
  }
  ...
  function drawPortal(g) {
    ...
    [102, 165].forEach(function (cx) {
      arch(g, cx, 595, 23, 455);
    });
    [472, 538].forEach(function (cx) {
      arch(g, cx, 595, 23, 455);
    });
    // chap gumbaz
  ...
  function drawIslomXoja(g) {
    ...
    [392, 442, 497, 547].forEach(function (ax) {
      arch(g, ax, 590, 18, 520);
    });
  ...
  function drawSkyline(g) {
    ...
    ln(g, 478, 508, 600, 508);
    ln(g, 600, 508, 600, 595);
    ...
    // kichik gumbazlar
    ln(g, 178, 540, 178, 502);
    ln(g, 232, 540, 232, 502);
    dome(g, 205, 502, 30, 33);
    ln(g, 405, 545, 405, 510);
    ln(g, 455, 545, 455, 510);
  ...
  function sortByAngle(pts, count) {
    ...
    idx.sort(function (a, b) {
      return (
        Math.atan2(pts[a * 2 + 1] - cy, pts[a * 2] - cx) -
  ...
  function init() {
    ...
    var SHAPES = [
      ...
      galaxyPoints(N), // portret yuklangunicha fallback
    ];
    ...
    var sprites = [
      makeSprite(
        "rgba(255,255,250,1)",
        "rgba(255,248,228,0.68)",
        "rgba(238,232,214,0.18)",
      ),
      makeSprite(
        "rgba(240,255,252,1)",
        "rgba(140,235,220,0.72)",
        "rgba(118,214,200,0.2)",
      ),
      makeSprite(
        "rgba(255,248,240,1)",
        "rgba(255,176,126,0.72)",
        "rgba(233,148,106,0.2)",
      ),
    ];
    var coreCols = [
      "rgba(255,253,244,",
    ...
    // uchar yulduzlar (pool: bir vaqtda 5 tagacha, tez-tez tug'iladi)
    var meteors = [],
      meteorAt = performance.now() + 1000 + Math.random() * 1200;
    var METEOR_MAX = 5;
    ...
    function frame(ts) {
      ...
      for (var n = 0; n < N; n++) {
        ...
        ctx.arc(
          p.x,
          p.y,
          Math.max(p.r * 0.62, 0.65) + glint * 1.0,
          0,
          Math.PI * 2,
        );
        ctx.fill();
      ...
      // ── uchar yulduzlar (yomg'ir) ──
      if (!rm) {
        if (meteors.length < METEOR_MAX && ts > meteorAt) {
          var fromLeft = Math.random() > 0.5;
          meteors.push({
            x: fromLeft ? -40 : vw + 40,
            y: vh * (0.04 + Math.random() * 0.5),
            vx: (fromLeft ? 1 : -1) * (7 + Math.random() * 5),
            vy: 2.2 + Math.random() * 1.6,
            life: 1,
          });
          meteorAt = ts + 900 + Math.random() * 1600;
        }
        for (var mi = meteors.length - 1; mi >= 0; mi--) {
          var meteor = meteors[mi];
          meteor.x += meteor.vx;
          meteor.y += meteor.vy;
          meteor.life -= 0.008;
          ctx.globalCompositeOperation = "lighter";
          ...
          var gr = ctx.createLinearGradient(
            meteor.x - meteor.vx * 11,
            meteor.y - meteor.vy * 11,
            meteor.x,
            meteor.y,
          );
      ...
      var capA =
        sfClamp(1 - Math.abs(sp.cur - near) * 2.4, 0, 1) * 0.5 * inten * settle;
      if (capA > 0.03) {
        ctx.globalCompositeOperation = "source-over";
