...
def comment_queryset(ct, object_id):
    ...
    ).select_related('author').prefetch_related('reactions').annotate(
...
def serialize_comment(comment, tg_profile_id=None):
    ...
    return {
        ...
        "reply_count": comment.reply_count if hasattr(comment, "reply_count") else comment.replies.filter(is_approved=True).count(),
